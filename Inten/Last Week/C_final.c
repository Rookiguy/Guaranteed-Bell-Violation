...
{  
   ...
   for(int k=0;k<51;k++){
      ...
      for(double w=0;w<1000;w++){  
         ...
         double theta3 = 2 * M_PI * u3;
         double phi3 = acos(2*v3-1);
         double x3   = sin(theta3)*cos(phi3);
         double y3   = sin(theta3)*sin(phi3);
         double z3   = cos(theta3);
         a3[0]=x3;
         a3[1]=y3;
         a3[2]=z3;
         ...
         double v4 = (((double)rand())/(double)RAND_MAX);
         double theta4 = 2 * M_PI * u4;
         double phi4 = acos(2*v4-1);
         double x4   = sin(theta4)*cos(phi4);
         double y4   = sin(theta4)*sin(phi4);
         double z4   = cos(theta4);
         b1[0]=x4;
         b1[1]=y4;
         b1[2]=z4;


         double u5 = (((double)rand())/(double)RAND_MAX);
         double v5 = (((double)rand())/(double)RAND_MAX);
         double theta5 = 2 * M_PI * u5;
         double phi5 = acos(2*v5-1);
         double x5   = sin(theta5)*cos(phi5);
         double y5   = sin(theta5)*sin(phi5);
         double z5   = cos(theta5);
         b2[0]=x5;
         b2[1]=y5;
         b2[2]=z5;


         double u6 = (((double)rand())/(double)RAND_MAX);
         double v6 = (((double)rand())/(double)RAND_MAX);
         double theta6 = 2 * M_PI * u6;
         double phi6 = acos(2*v6-1);
         double x6   = sin(theta6)*cos(phi6);
         double y6   = sin(theta6)*sin(phi6);
         double z6   = cos(theta6);
         b3[0]=x6;
         b3[1]=y6;
         b3[2]=z6;
         //printf("[%lf,%lf,%lf],\n",x,y,z);
